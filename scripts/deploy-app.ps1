...
param(
    # Project name, if not specified, the script will use the PROJECT_NAME environment variable
    [Parameter(Mandatory=$false)]
    [Alias("project")]
    [string]$projectName = $env:PROJECT_NAME,
    # Service name, if not specified, the script will use the CONTAINER_NAME environment variable
    [Parameter(Mandatory=$false)]
    [Alias("service-name")]
    [string]$serviceName = $env:CONTAINER_NAME,
...
# Validations
if (-not $serviceName) {
    Write-Host "Service name not specified. Exiting..." -ForegroundColor Red
    exit
}
if (-not $projectName) {
    Write-Host "Project name not specified. Exiting..." -ForegroundColor Red
    exit
}
...
$registryName = $env:PROJECT_NAME
$imageName = $serviceName
$numberOfImagesToKeep = 2

# Azure Container App
$resourceGroup = $env:PROJECT_NAME
